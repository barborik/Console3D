...
public class Main {
    ...
    public static void renderObj(double[][] verts) {
        render.start();
        while (true) {
            double[][] rotationX = {
                    {1, 0, 0},
            ...
            for (int i = 0; i < vertsToRender.length; i += 4) {
                ...
                //drawTriangle(vertsToRender[i], vertsToRender[i + 1], vertsToRender[i + 2]);
            }
    ...
    public static double[][] loadObject(String filename) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        double[][] debugTriangle = {
                {-0.5, -0.5, 0.5}, {0, 0.5, 0.5}, {0.5, -0.5, 0.5},
                {-0.5, -0.5, -0.5}, {0.5, -0.5, -0.5}, {0, 0.5, -0.5}
        };
        //renderObj(debugTriangle);

        cols = Integer.parseInt(args[1]);
        rows = Integer.parseInt(args[2]);
        screenBuffer = new char[2][rows][cols + 1];

        System.out.print("\033[?25l\033[2J"); // hides the cursor and clears the screen

        initScreen(screenBuffer[0]);
        initScreen(screenBuffer[1]);
        renderObj(loadObject(args[0]));
    }
}
